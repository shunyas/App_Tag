...
/**
 * ネットワークの初期化を行う
 * @param E_STATE_APP_WAIT_NW_START
 * @param pEv
 * @param eEvent
 * @param u32evarg
 */
PRSEV_HANDLER_DEF(E_STATE_APP_WAIT_NW_START, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	...
	if (sAppData.u8NwkStat == E_IO_TIMER_NWK_NWK_START) {
...
PRSEV_HANDLER_DEF(E_STATE_APP_WAIT_TX, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if (eEvent == E_EVENT_NEW_STATE) {
		...
		S_OCTET(PKT_ID_IO_TIMER); // パケット識別子
...
static const tsToCoNet_Event_StateHandler asStateFuncTbl[] = {
	...
	PRSEV_HANDLER_TBL_DEF(E_STATE_APP_WAIT_NW_START),
