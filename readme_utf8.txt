...
・共通設定
  ...
  Option Bits     : オプションビット
    ...
    0x00000020 温湿度センサー SHT21 の稼働
               Parent で設定するとログ出力を SimpleTagV3 形式で出力

    0x00000100 UART 入力直後に送信
               DIO12 により起床⇒DIO6 が LO になったらデータ入力可
               100ms でタイムアウト

    0x00000200 バイナリ書式モード (標準は ASCII 書式モード)
               Parent の出力様式も同様
...
■ 子機(Input)
子機はオプションビットによって幾つかの振る舞いが定義されています。
...
SHT21 モード
  ...
  SHT21 モードでは I2C の SHT2x による温湿度計測を行います。設定 [Sleep Dur] の期間
  毎に起床して、センサーを稼働させ、データを送信します。

＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
UART 送信モード

  UART 送信モードでは、スリープ状態にある子機を IO 割り込みにより起床させ、
  起床後、送信コマンドを受けつけ、コマンド受け付け後に、データ送信を行い
  再びスリープします。

  UART 送信ではオプションビットの 0x0000:0100 をセットします。

  動作シーケンス（電源投入時）
  ・電源投入⇒速やかにスリープ (10ms 程度)

  動作シーケンス（スリープ復帰時）
  ・スリープ復帰は DIO12 の H>L 割り込み
  ・UART 受けつけ可能になったら速やかに DIO5 を LO にする (RTS ピンとして振る舞う)
    同時に :8100436DCF といったメッセージを出力
      (: + モジュールのシリアル番号 + チェックサム)
  ・復帰後は、設定 [Sleep Dur] の期間はコマンド入力を待つが、入力が無ければ、
    そのままスリープ。
  ・UART からの入力コマンド (App_UART の ASCII 形式) を受けつけ後、データ送信を
    行う。
    データ例 :00112233X (X はチェックサム, CRLF の省略形)
  ・データ送信が完了したらスリープに遷移
    送信完了メッセージは出力していません。
